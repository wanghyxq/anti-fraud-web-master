              ...
              <div class="scroll-wrapper">
                <div v-for="(item, index) in doubleUrlData" :key="index" class="url-row">
                  ...
                  <div class="url-content">
                    <div class="rank-num" 
                         :class="'rank-' + ((index % maliciousUrlData.length) + 1)">
                      NO.{{ (index % maliciousUrlData.length) + 1 }}
                    </div>
...
.scroll-wrapper {
  animation: scrollUp 20s linear infinite; /* 稍微放慢速度，看起来更稳重 */
}
...
/* --- 新增：排名序号样式 --- */
.rank-num {
  font-family: 'DigifaceWide', sans-serif; /* 使用数字字体 */
  font-size: 18px;
  font-weight: bold;
  margin-right: 15px;
  width: 50px;
  text-align: left;
  font-style: italic; /* 斜体更有速度感 */
}
/* 前三名特殊配色 */
.rank-1 { color: #FFD700; text-shadow: 0 0 10px #FFD700; } /* 金 */
.rank-2 { color: #C0C0C0; text-shadow: 0 0 10px #C0C0C0; } /* 银 */
.rank-3 { color: #CD7F32; text-shadow: 0 0 10px #CD7F32; } /* 铜 */
/* 其他名次 */
...
.bar-fill {
  height: 100%;
  background: linear-gradient(90deg, rgba(0,126,255,0.8) 0%, rgba(1,255,246,1) 100%);
  border-radius: 3px;
  position: relative;
  box-shadow: 0 0 8px rgba(1, 255, 246, 0.4);
}
