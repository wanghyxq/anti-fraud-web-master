...
export default {
  ...
  mounted() {
    ...
    this.$nextTick(() => {
      ...
      // this.initTrendChart(); // 新增：调用趋势图初始化
      this.getTrendData(); // 不再直接 initTrendChart，调用获取数据的方法
    ...
    getTrendData() {
      ...
      request({
        ...
        if (response.code === 200 && response.data) {
          ...
          // 3. 追加到数组末尾
          this.trendData = [...data, twentyFourHourItem];
      ...
      const option = {
        ...
        xAxis: {
          ...
          axisLabel: {
            ...
            lineHeight: 18,                
            margin: 15,                    
            // 间隔逻辑：每4小时显示一次 (0, 4, 8 ... 24)
            interval: (index) => index % 4 === 0,                
            // 格式化：增加“时”
    ...
    handleResize() {
      ...
      // 加入 resize（折线图）
        this.$nextTick(() => {
            ...
            if (this.trendChartInstance) this.trendChartInstance.resize(); // 新增这行
        })
    },
    handleNav(type) {
